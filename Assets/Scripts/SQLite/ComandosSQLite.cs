...
public class ComandosSQLite : MonoBehaviour
{
    ...
    public static ComandosSQLite Instance
    {
        ...
        {
            if (instance == null)
                instance = FindObjectOfType(typeof(ComandosSQLite)) as ComandosSQLite;
            return instance;
        }
        set
        {
    ...
    void ComandoSelect(string item, string tabla)
    {
        ...
        while (reader.Read())
        {
            try
            {
                Debug.Log("Kills: " + reader.GetInt32(0));
            }
    ...
    public void AddKillToDB()
    {
        AbrirDB();
        // Crear la consulta
        dbCommand = dbConnection.CreateCommand();
        string sqlQuery = "UPDATE Kills SET Kills = Kills + 1";
        dbCommand.CommandText = sqlQuery;
