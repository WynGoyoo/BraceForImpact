...
{
    public class TimedObectActivated : MonoBehaviour
    {
        public enum Action
        {
            Activate,
...
{
#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(TimedObectActivated.Entries))]
    public class EntriessDrawer : PropertyDrawer
    {
            ...
            if (entries.arraySize > 0)
            {
                ...
                for (int i = 0; i < entries.arraySize; ++i)
                {
                    ...
                    if (entry.FindPropertyRelative("action").enumValueIndex !=
                        (int)TimedObectActivated.Action.ReloadLevel)
                    {
