...
public class SplineInspector : Editor
{
	...
	private void AddSplineNode( )
	{
		...
		gObject.name = GetNodeName( spline.gameObject.transform.childCount );
		...
		foreach( Transform node in splineNodes )
		{
			...
			case Tool.Rotate:
				...
				//Handles.ArrowCap( 0, node.position, Quaternion.LookRotation( node.forward ), HandleUtility.GetHandleSize( node.position ) * 0.5f );
			     UnityEditor.Handles.ArrowHandleCap(0, node.position, Quaternion.LookRotation(node.forward), HandleUtility.GetHandleSize(node.position) * 0.5f, Event.current.type);
			...
			case Tool.Scale: case Tool.Move:
				CreateSnapshot( );
